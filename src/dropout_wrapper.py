...
class DropoutWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if len(x.size()) == 3:
            ...
            mask = Variable(1.0 / (1-self.prob) * torch.bernoulli((1-self.prob) * (x.data.new(x.size(0), x.size(2)).zero_() + 1)), requires_grad=False) # Use bernoulli distribution to approximate which elements to consider
