...
if not args.san_off:
    if args.lambda_comp:
        ...
        for dataset in datas:
            for i,metric in enumerate([f'{dataset}_em', f'{dataset}_f1']):
                plt.gcf().clear()
                plt.title(f' {dataset} {metric.split("_")[-1]} for only SAN')
                plt.xlabel('epochs')
    ...
    for lval in lambda_vals:
        ...
        plt.legend(loc='upper left')
        plt.title(f'Training loss for only SAN, lambda = {lval}')
    ...
    if args.lambda_comp:
        ...
        for dataset in datas:
            for i,metric in enumerate([f'{dataset}_acc', f'{dataset}_em', f'{dataset}_f1']):
                plt.gcf().clear()
                plt.title(f' {dataset} {metric.split("_")[-1]} for Joint SAN + classifier')
                plt.xlabel('epochs')
                plt.gca().set_ylim([0,100])
                for lval in lambda_vals:
